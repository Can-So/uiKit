import { p, blockquote, panel, emoji, mention, hardBreak, code_block, h1, ul, ol, li, hr, img, date, decisionList, decisionItem, taskList, taskItem, doc, table, td, tr, th, extension, inlineExtension, bodiedExtension, media, mediaSingle, mediaGroup, applicationCard, em, strong, code, strike, underline, a as link, subsup, textColor, text, inlineCard, blockCard, status, } from './schema-builder';
export var createText = function (txt) { return function (schema) {
    return text(txt, schema);
}; };
export var pmNodeFactory = {
    doc: doc,
    paragraph: p,
    ...
    emoji: function () { return emoji({ shortName: 'fakeName' }); },
    mention: function () {
        return mention({
            id: '1234',
            text: 'fakeMention',
    ...
    image: function () { return img({ src: 'src/testsource.png' }); },
    date: function () { return date({ timestamp: '121220121212' }); },
    status: function () {
        return status({
            color: 'yellow',
            localId: 'fake-status',
    ...
    table: function (content) { return table()(content); },
    ...
    inlineCard: function () {
        return inlineCard({ url: 'https://product-fabric.atlassian.net/browse/ED-1' });
    },
    blockCard: function () {
        return blockCard({ url: 'https://product-fabric.atlassian.net/browse/ED-1' });
    },
};
export var pmNodeBuilder = {
    doc: doc(p('')),
    ...
    decisionList: decisionList({ localId: 'fake-decision-list' })(decisionItem({ localId: 'fake-decision-item' })('fake decision')),
    taskList: taskList({ localId: 'fake-task-list' })(taskItem({ localId: 'fake-task-item' })('fake task')),
    ...
    table: table()(tr(th({ colspan: 1, rowspan: 1 })(p('fake table header'))), tr(td({ colspan: 1, rowspan: 1 })(p('fake table row')))),
    ...
    mediaSingle: mediaSingle({ layout: 'center' })(media({
