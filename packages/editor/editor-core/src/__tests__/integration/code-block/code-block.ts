...
BrowserTestCase(
  'code-block: code block language is preserved after floating toolbar loses and gains focus',
  { skip: ['ie', 'safari'] },
  async client => {
    const page = new Page(client);
    await page.goto(fullpage.path);
    ...
    await page.type(fullpage.placeholder, ['ArrowRight', 'ArrowRight']);
    await page.type(fullpage.placeholder, 'test paragraph');
...
BrowserTestCase(
  'code-block: code block selected language correctly changes when moving selection directly from one code block to another',
  { skip: ['ie', 'safari'] },
  async client => {
    const page = new Page(client);
    await page.goto(fullpage.path);
    ...
    await page.type(fullpage.placeholder, ['ArrowDown']);
    ...
    await page.waitForSelector(floatingToolbarLanguageSelector);
    const firstCodeBlock = await page.getText(floatingToolbarLanguageSelector);
    ...
    await page.waitForSelector(floatingToolbarLanguageSelector);
    const secondCodeBlock = await page.getText(floatingToolbarLanguageSelector);
