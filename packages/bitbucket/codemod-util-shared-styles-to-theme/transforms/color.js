...
export default function colorTransform(root, j) {
  ...
  var colorImportSpecifiers = sharedStylesImport.find(j.ImportSpecifier).filter(function (spec) {
    return Object.keys(colorMappings).includes(spec.node.imported.name);
  ...
  var importedColors = [];
  colorImportSpecifiers.forEach(function (spec) {
    importedColors.push({
      local: spec.node.local.name,
      imported: spec.node.imported.name
    });
  });
  colorImportSpecifiers.remove(); // Convert the colors usage to colors.*

  importedColors.forEach(function (spec) {
    root.find(j.Identifier, {
      name: spec.local
    }).replaceWith(function () {
      return j.identifier(colorMappings[spec.imported]);
    });
  }); // Add an import of colors from @atlaskit/theme

  addNamedImport(root, j, '@atlaskit/theme', 'colors', 'colors', sharedStylesImport); // Remove util shared styles if no named imports
