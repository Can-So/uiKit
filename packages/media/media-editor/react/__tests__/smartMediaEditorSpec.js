...
import { asMock, expectFunctionToHaveBeenCalledWith, expectToEqual, fakeContext, } from '@atlaskit/media-test-helpers';
...
describe('Smart Media Editor', function () {
    ...
    beforeEach(function () {
        formatMessage = jest
            .fn()
            .mockImplementation(function (message) { return message.defaultMessage; });
    ...
    describe('when changes has been made and cancel is pressed', function () {
        var modalDialog;
        beforeEach(function () { return tslib_1.__awaiter(_this, void 0, void 0, function () {
            var editorView, _a, onAnyEdit, onCancel;
            return tslib_1.__generator(this, function (_b) {
                switch (_b.label) {
                    ...
                    case 1:
                        _b.sent();
                        editorView = component.find(EditorView);
                        _a = editorView.props(), onAnyEdit = _a.onAnyEdit, onCancel = _a.onCancel;
                        onAnyEdit();
                        onCancel();
        ...
        it('should show confirmation dialog when user cancels', function () {
            expect(modalDialog).toHaveLength(1);
            expect(modalDialog.prop('heading')).toEqual('Unsaved changes');
        });
        it('should call onFinish when first action is chosen', function () {
            var firstAction = modalDialog.prop('actions')[0];
            expect(firstAction.text).toEqual('Close anyway');
            firstAction.onClick();
            expect(onFinish).toHaveBeenCalled();
        });
        it('should just close confirmation dialog and not call onFinish when second action is chosen', function () {
            var secondAction = modalDialog.prop('actions')[1];
