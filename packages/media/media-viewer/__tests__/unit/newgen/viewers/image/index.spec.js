...
function createFixture(response, item) {
    if (item === void 0) { item = imageItem; }
    var context = fakeContext();
    context.getImage.mockReturnValue(response);
    var onClose = jest.fn();
    var onLoaded = jest.fn();
    var el = mountWithIntlContext(React.createElement(ImageViewer, { context: context, item: item, collectionName: collectionName, onClose: onClose, onLoad: onLoaded }));
...
describe('ImageViewer', function () {
    ...
    it('should not call context.getImage when image representation is not present', function () { return tslib_1.__awaiter(_this, void 0, void 0, function () {
        var response, _a, el, context;
        return tslib_1.__generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    response = Promise.resolve(new Blob());
                    _a = createFixture(response, tslib_1.__assign({}, imageItem, { representations: {} })), el = _a.el, context = _a.context;
                    return [4 /*yield*/, response];
                case 1:
                    _b.sent();
                    el.update();
